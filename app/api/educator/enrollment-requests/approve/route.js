...
export async function POST(request) {
    try {
        ...
        const student = await User.findById(enrollmentRequest.userId);
        if (!student) {
            return NextResponse.json({ success: false, message: 'Student not found' });
        }

        // If already completed/enrolled, short-circuit
        if (enrollmentRequest.status === 'completed') {
            return NextResponse.json({ success: true, message: 'Student already enrolled' });
        }
